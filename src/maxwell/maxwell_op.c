...
/* Compute Yout from Yin using the first order Maxwell operator */
void maxwell_operator_(evectmatrix Yin, evectmatrix Yout, void *data,
		      int is_current_eigenvector, evectmatrix Work)
{
     maxwell_data *d = (maxwell_data *) data;
     int cur_band_start;
     scalar_complex *cdata;
     real scale;
     evectmatrix Hin, Din, Hout, Dout;

	 Hin=create_evectmatrix(Yin.N, Yin.c, Yin.p/2,
			       Yin.localN, Yin.Nstart, Yin.allocN);
	 Din=create_evectmatrix(Yin.N, Yin.c, Yin.p/2,
			       Yin.localN, Yin.Nstart, Yin.allocN);
	 Hout=create_evectmatrix(Yin.N, Yin.c, Yin.p/2,
			       Yin.localN, Yin.Nstart, Yin.allocN);
	 Dout=create_evectmatrix(Yin.N, Yin.c, Yin.p/2,
			       Yin.localN, Yin.Nstart, Yin.allocN);

	 evectmatrix_copy_slice(Yin, Hin, 0, 0, Yin.p/2);
	 evectmatrix_copy_slice(Yin, Din, Yin.p/2, 0, Yin.p/2);

     
     CHECK(d, "null maxwell data pointer!");
     CHECK(Yin.c == 2, "fields don't have 2 components!");

     (void) is_current_eigenvector;  /* unused */
     (void) Work;

     cdata = (scalar_complex *) d->fft_data;
     scale = -1.0 / Yout.N;  /* scale factor to normalize FFT; 
				negative sign comes from 2 i's from curls */

     /* compute the operator, num_fft_bands at a time: */
     for (cur_band_start = 0; cur_band_start < Din.p; 
	  cur_band_start += d->num_fft_bands) {
	  int cur_num_bands = MIN2(d->num_fft_bands, Din.p - cur_band_start);


      maxwell_compute_d_from_H(d, Hin, Dout.data,
                                       cur_band_start, cur_num_bands);

	  maxwell_compute_e_from_d(d, Din.data, cur_num_bands);
	  maxwell_compute_H_from_e(d, Hout, Din.data,
				   cur_band_start, cur_num_bands, scale);

     }
	 evectmatrix_copy_slice(Hout, Yout, 0, 0, Yin.p/2);
	 evectmatrix_copy_slice(Dout, Yout, 0, Yin.p/2, Yin.p/2);
}
