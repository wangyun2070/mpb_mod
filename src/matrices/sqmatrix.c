...
void sqmatrix_resize(sqmatrix *A, int p, short preserve_data)
{
     ...
     if (preserve_data) {
	  ...
	  /*A->data is a one-dimensional array of size p*p, with the first p elements as 
	  the first row, the second p elements as the second row, ... which is actually
	  a matrix of size p*p */
	  //If p < A->p, only keep the upper left p elements
	  //If p > A->p, keep all the elements of A->data, and leave all other elements blank
	  if (p < A->p) {
	       for (i = 0; i < p; ++i)
		    for (j = 0; j < p; ++j)
	 ...
	 //Indicate A->data is now of size p*p
     A->p = p;
}
...
//It seems that the eigenvectors are stored in U which is overwritten.
...
/* Compute Usqrt <- sqrt(U), where U must be Hermitian positive-definite. 
   ...
   W are overwritten. */
//sqrt a matrix by sqrt the eigenvalues of it.
void sqmatrix_sqrt(sqmatrix Usqrt, sqmatrix U, sqmatrix W)
{
     ...
     sqmatrix_eigensolve(U, eigenvals, W);  //Find the eigenvals of U, W is a work matrix
