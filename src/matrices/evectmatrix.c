			...
			sqmatrix S)
{
     ...
     memset(S.data, 0, sizeof(scalar) * (Y.p * Y.p));//Start from S.data, replace sizeof(scalar) * (Y.p * Y.p) data with 0
     blasglue_gemm('C', 'N', X.p, X.p, X.n,
		   1.0, X.data, X.p, Y.data, Y.p, 0.0, S.data, Y.p);//S.data=X.data^dagger * Y.data
     ...
     for (i = 0; i < Y.p; ++i) {
	  mpi_allreduce(S.data + i*Y.p, U.data + Uoffset + i*U.p, 
			Y.p * SCALAR_NUMVALS,
			real, SCALAR_MPI_TYPE, MPI_SUM, mpb_comm);
     }//It seems like this just copies S.data to U.data (?)
}
