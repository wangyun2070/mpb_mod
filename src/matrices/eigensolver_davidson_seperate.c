...
void eigensolver_davidson(evectmatrix Y, evectmatrix Z, real *eigenvals,
			  evectoperator A, void *Adata,
			  ...
			  real target)
{
	 ...
	 ndata should be doubled while keeping each elements of Work invariant*/
     nbasis = nWork / 4;
     V = Work;                      //H field
     AV = Work + nbasis;			//The result after applying the maxwell operator on H field
	 V_z = Work + 2 * nbasis;		//D field
	 AV_z = Work + 3 * nbasis;		//The result after applying the maxwell operator on D field

     q = Y.p * nbasis * 2;           //While calculating VAV, S, Swork, we should consider D and H fields together
     ...
     do {
	  ...
	  A(V_z[ibasis], AV_z[ibasis], V[ibasis], AV[ibasis], Adata, 0, Y;)  

	  q = Y.p * (ibasis + 1) * 2;    //Consider D and H fields together
