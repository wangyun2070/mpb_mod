...
void init_params(integer p, boolean reset_fields)
{
	...
	doing and what we've read in. */
     ...
     if (!have_old_fields) {
	  ...
	  D = create_evectmatrix(nx * ny * nz, 2, num_bands,
				 local_N, N_start, alloc_N);
	  nwork_alloc = eigensolver_nwork + (mdata->mu_inv!=NULL);
	  for (i = 0; i < nwork_alloc; ++i)
	       W[i] = create_evectmatrix(nx * ny * nz, 2, block_size*2,
					 local_N, N_start, alloc_N);
	  if (block_size < num_bands){
	       Hblock = create_evectmatrix(nx * ny * nz, 2, block_size,
					   local_N, N_start, alloc_N);
		   Dblock = create_evectmatrix(nx * ny * nz, 2, block_size,
					   local_N, N_start, alloc_N);
	  }
	  else{
	       Hblock = H;
...
void solve_kpoint(vector3 kvector)
{
	...
	for the frequency grep data: */
     ...
     CHK_MALLOC(eigvals, real, num_bands*2);
     ...
     if (mdata->zero_k && !mtdata) {
	  ...
	  for (in = 0; in < H.n; ++in){
	       for (ip = 0; ip < H.p - ib0; ++ip){
			...
			D.data[in * D.p + ip] = D.data[in * D.p + ip + ib0];
		   }
	  }
	  evectmatrix_resize(&H, H.p - ib0, 1);
	  evectmatrix_resize(&D, D.p - ib0, 1);
     }
     else
     ...
     for (ib = ib0; ib < num_bands; ib += Hblock.alloc_p) {
	  ...
	  if (ib + mdata->num_bands > num_bands) {
	       ...
	       for (i = 0; i < nwork_alloc; ++i)						//How is W[i] used in the calculation?
		    evectmatrix_resize(&W[i], 2*(num_bands - ib), 0);      //A larger size of the matrix for both H and D fields                         
	       evectmatrix_resize(&Hblock, num_bands - ib, 0);
		   evectmatrix_resize(&Dblock, num_bands - ib, 0);
	  }
	  ...
	  if (Hblock.data != H.data) {  /* initialize fields of block from H */
	       ...
	       deflation.p = ib-ib0;
	       if (deflation.p > 0) {                                                    //What does this deflaction do?
	  ...
	  if (Dblock.data != D.data) {  /* initialize fields of block from D */
	       int in, ip;
	       for (in = 0; in < Dblock.n; ++in)
		    for (ip = 0; ip < Dblock.p; ++ip)
			 Dblock.data[in * Dblock.p + ip] =
			      D.data[in * D.p + ip + (ib-ib0)];
	      /* deflation.p = ib-ib0;
	       if (deflation.p > 0) {                                                    //What does this deflation do?
                    if (deflation.BY.data != H.data) {
                        evectmatrix_resize(&deflation.BY, deflation.p, 0);
                        maxwell_muinv_operator(H, deflation.BY, (void *) mdata,
                                               1, deflation.BY);
	  ...
	  if (mtdata) {  /* solving for bands near a target frequency */
		  ...
		  eigenvectors: */
		   ...
		   /* eigensolver_davidson(
			 Hblock, Dblock, eigvals + ib,
			 maxwell_operator_, (void *) mdata,
			 simple_preconditionerp ?
